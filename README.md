...
## How It Works
The AI works by using a simple neural network with 3 inputs (y_movement, height_of_top_pipe, height_of_bottom_pipe) it could theoraticly work with just 2 input but that would require more computer power since the neural network would need to calculate the distance between each pipe therfore increasing training times. The neurel network only has one output, should the bird go up or down. The bird only needs to move up or down becasue it does not move forward instead the environment moves backward.	